...
int main(int argc, char* argv[]) {
	...
	for (int id=0; id<timeStamp; id++){ // diffrent dimension
		Params par(dim[id],dim[id], 1,0.005,50,1600);
  ...
  ofs << "plt.plot(size,v1, label='kokkod 0')\n";
  ofs << "plt.plot(size,v2, label='kokkod 1')\n";
  ofs << "plt.plot(size,v3, label='kokkod 2')\n";

  ofs << "plt.grid(True)\n";

  ofs << "plt.title('draft')\n";
  ofs << "plt.xlabel('Time Stamp')\n";
...
//     	// computing time
//     	par.Time = it * par.dt;
// 		Kokkos::parallel_for( team_policy_t(nbTeams, 
//                     Kokkos::AUTO, /* team size chosen by kokkos */
//                     team_policy_t::vector_length_max()) , KOKKOS_LAMBDA(const thread_t& thread) 
// 		{  
// 			int index = thread.league_size();
//     		int i,j;
// 			index2coord(index,i,j,par.NX,par.NY);
